...
def illustrate_driving_lane(img, left_line, right_line, lane_color=(255, 0, 255), road_color=(0, 255, 0)):
    ...
    cv2.fillPoly(window_img, [np.array(left_line_pts, dtype=np.int32)], lane_color)
    cv2.fillPoly(window_img, [np.array(right_line_pts, dtype=np.int32)], lane_color)
    ...
    # Draw the lane onto the warped blank image
    # HATALI SATIR: cv2.fillPoly(window_img, int([pts]), road_color)
    # DÜZELTILMIŞ HALİ:
    cv2.fillPoly(window_img, [np.array(pts, dtype=np.int32)], road_color)
...
def illustrate_driving_lane_with_topdownview(image, left_line, right_line):
    ...
    # Draw the lane onto the warped blank image
    # HATALI SATIRLAR:
    # cv2.fillPoly(window_img, int([left_line_pts]), (140, 0, 170))
    # cv2.fillPoly(window_img, int([right_line_pts]), (140, 0, 170))
    
    # DÜZELTILMIŞ HALİ:
    cv2.fillPoly(window_img, [np.array(left_line_pts, dtype=np.int32)], (140, 0, 170))
    cv2.fillPoly(window_img, [np.array(right_line_pts, dtype=np.int32)], (140, 0, 170))
    ...
    # Draw the lane onto the warped blank image
    # HATALI SATIR:
    # cv2.fillPoly(window_img, int([pts]), (0, 160, 0))
    
    # DÜZELTILMIŞ HALİ:
    cv2.fillPoly(window_img, [np.array(pts, dtype=np.int32)], (0, 160, 0))
